...
#Top 3 federal: CA, TX, NY, and FL
fig1_data <- states_ash %>%
...
#Top 3 local: NY, CA, TX, IL
fig2_data <- states_ash %>%
  mutate(rev_mil = mean_revenue/1000000) %>%
  filter(type == "Local") %>%
  group_by(state, year) %>%
  summarise(mean_rev_mil = mean(rev_mil, na.rm = TRUE)) %>%
...
ggplot(fig1_data, aes(x = year, y = mean_rev_mil, color = state)) +
  geom_line(size = 0.75) +
  gghighlight(state == "California" |
              state == "New York" |
              state == "Texas",
...
ggplot(fig2_data, aes(x = year, y = mean_rev_mil, color = state)) +
  geom_line(size = 0.75) +
  gghighlight(state == "California" |
              state == "New York" |
              state == "Texas",
...
ggplot(fig3_data, aes(x = year, y = mean_rev_mil, color = state)) +
  geom_line(size = 0.75) +
  gghighlight(state == "California" |
              state == "New York" |
              state == "Texas",
