...
It appears that coloring by region is pretty useless... so instead, I want to highlight the top 3 states in each condition (federal, state, local).
...
states_ash %>%
  ...
  ggplot(aes(x = year, y = mean_revenue_mil, color = state)) +
    geom_smooth(size = 0.85, se = FALSE) + #when I use geom_line, it doesn't work as I intended
    facet_wrap(~type) +
...
Figure2 <- plot_grid(fig1, fig2, fig3, labels = c("", "", ""), align = "h", nrow = 1)
